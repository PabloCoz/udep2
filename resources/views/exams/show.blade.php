<x-app-layout>
    <div class="max-w-6xl mx-auto py-8 md:py-16">
        <div class="block md:flex justify-between mb-5 mx-3 md:mx-0">
            @livewire('exam.users', ['exam' => $exam]/* , key($exam->id) */)
            <a href="{{route('pdf', $exam)}}" class="flex items-center space-x-3 text-center text-white bg-red-500 px-3 py-2 rounded shadow-lg shadow-red-500/75">
                            ...
                            <div class="flex space-x-5 items-center">
                                @foreach ($question->options as $option)
                                    <div class="flex items-center space-x-1">
                                        ...
                                        <input type="checkbox" class="rounded" name="{{ $option->name }}" id="">
                                    </div>
                                @endforeach
                            </div>
                        </div>
